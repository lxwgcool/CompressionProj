...
void ClsMethod::UnAlignedMiniReadsComprByHuffmanCoding(string strFasta, int iKmerLen)
{
    ...
    {        
        ...
        for(vector<St_SuperKmer>::iterator itr = vSuperKmer.begin(); itr != vSuperKmer.end(); itr++)
        {
            ...
            {
                itr->mpOrgSimilar[itrCandi->strSeq]++;
                bFind = true;
                break;
            }
        }
    ...
    int iCurSize = vSuperKmer.size();
    for(vector<St_KmerInfo>::iterator itrReg = vKmerRegular.begin();
        itrReg != vKmerRegular.end(); itrReg++)
    {
        //同样的逻辑 --> 从头到尾进行遍历，然后进行similar的查找，直到所有元素都遍历完成
        bool bFind = false;
        for(vector<St_SuperKmer>::iterator itr = vSuperKmer.begin() + iCurSize;
    ...
    //Step4: Build Huffman Tree for SuperKmer --> Double Level --> Go!!
    //(1) Init Huffman Tree -->
    map<string, int> mpKmerFreq;
    for(vector<St_SuperKmer>::iterator itr = vSuperKmer.begin();
        itr != vSuperKmer.end(); itr++)
    {
        mpKmerFreq[itr->stInfo.strSeq] = itr->GetFreqOrg();
    }

    Hufftree<string, int>* hufftree = NULL;
    hufftree = new Hufftree<string, int>(mpKmerFreq.begin(), mpKmerFreq.end());

    //Step5: Compress the sequence by huffman coding Huffman Coding
    vector<bool> vEncodedLongGens = hufftree->encode(vGens.begin(), vGens.end());
    ...
    writeBits( destBf, &vEncodedLongGens );
    destBf.ByteAlign();
