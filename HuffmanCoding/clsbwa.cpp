...
string ClsBWA::CreateBamBySingleReads(string& strRefPath, string& strReadsPath,
                                      ...
                                      bool bSortByName, int iThreadNum)
{
    string strRootPath = ::GetHigherFolderPath(get_current_dir_name());
    string strBWAPath = strRootPath + "ThirdPartyTools/bwa/bwa";
    if(strBamFolder == "")
        strRootPath += "TempFile/";
    else
    {
        strRootPath += strBamFolder;
        if(strBamFileName != "")  // 如果之前已经坐过了，那么我们据不再做了
        {
    ...
    //Normal sorted
    if(bSortByName)
        strCmd = "../Bamtools/bin/bamtools sort -in " + strBamPath + " -out " +
                 strSortedBamPath + " -byname";
    else
        strCmd = "../Bamtools/bin/bamtools sort -in " + strBamPath + " -out " + strSortedBamPath;
    system(strCmd.c_str());

    //7:build index file for bam file
    strCmd = "../Bamtools/bin/bamtools index -in " + strSortedBamPath;
    system(strCmd.c_str());
...
string ClsBWA::CreateBamByPEReads(string& strRefPath, string& strReads1Path, string& strReads2Path,                                  
                                  ...
                                  bool bMapAll, bool bLooseMatch,
                                  string strBamFileNameCustmize, int iThreadNum)
{
    string strCmd = "";
    string strRootPath = GetHigherFolderPath(get_current_dir_name());
    string strBWAPath = strRootPath + "ThirdPartyTools/bwa/bwa";
    ...
    if(bSortByName)
        strCmd = "../Bamtools/bin/bamtools sort -in " +
