...
void encoder(string ifile, string ofile, bool verbose)
{
    ...
    {
        string strRmExistFile = "rm " + ofile;
        ::system(strRmExistFile.c_str());
...
int Test_HuffmanCode( int argc, char * argv[] )
{
    /*
    //--> Try to write one bit into File
    string strValue= "";
    char cValue = -1;
    long lValue = strtol(strValue.c_str(), NULL, 2);
    unsigned char b = lValue & 0xffl;
    int fdfd = 0xf;

    string strFilePath = "/home/lq/lxwg/WorkStudio/Prototype/Scaf_Gap_Filling_Tools/Huffman_Coding/Test/Output_Test";
    ofstream ofs;
    ofs.open(strFilePath.c_str(), ios::out|ios::binary);
