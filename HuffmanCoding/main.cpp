                                  ...
                                  //-> We find Kmer = 38 could give us the best result
{
    ...
    map<int, string>::iterator itr = mpTest.begin();
    itr++;
    itr++;
    for(; itr != mpTest.end(); itr++)
    {
        map<int, string>::iterator itrDelete = itr;  //Tips!!!!!! --> Should use another itr to set the value for deletion
        itr++;
        mpTest.erase(itrDelete);
        itr--;
    }
    ...
    vTest.push_back(5);
    vTest.push_back(6);
    for(vector<int>::iterator itr = vTest.begin()+2; itr != vTest.end(); itr++)
    {
        vTest.erase(itr);
        itr--;
    }
